"""
MLP Training and Evaluation Script

This script trains and evaluates an MLP model on the Chinese MNIST dataset.
...
if __name__ == "__main__":
    ...
    # Initialize model
    my_model = MLPBaseline(input_dim=input_dim, num_classes=my_num_classes, dropout=0.5).to(my_device)
    ...
    # Optimizer choice
    if OPTIMIZER == "sgd":
        my_optimizer = SGD(my_model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY, momentum=0.9)
    else:
        my_optimizer = Adam(my_model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
