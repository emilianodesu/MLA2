...
def train_mlp_model(input_dim,
    ...
    if path is not None:
        ...
        ckpt_path = Path("checkpoints/mlp_model_2.pth")
    ...
    if input_dim is None:
        try:
            ...
            raise RuntimeError("Unable to infer input dimension from dataloader/dataset.") from exc
    ...
    my_model = MLPBaseline(input_dim=input_dim, num_classes=num_classes, dropout=0.5).to(my_device)
    ...
    for epoch in range(1, epochs + 1):
        train_loss, train_acc = train_one_epoch(my_model, my_dataloaders['train'], my_optimizer, my_device)
        val_loss, val_acc, _, _, _ = evaluate(my_model, my_dataloaders['val'], my_device)
...
if __name__ == "__main__":
    ...
    model_path, model, device, dataloaders = train_mlp_model(
        input_dim=INPUT_DIM,
    ...
    test(model_path, model, device, dataloaders)
